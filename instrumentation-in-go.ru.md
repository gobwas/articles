...
В этой статье я хотел бы поделиться подходом к профилированию и трассировке
программ на Go. Я расскажу, как это можно сделать сохранив код гибким и чистым.
...
б_о_льшая часть его кода будет содержать код трассировки, а не код его основной
...
измеренять работу нашего компонента любым способом.
...
возвращать callback. `OnPing()` будет вызван непосредственно перед выполнением
пинга, а callback – сразу после. Таким образом мы сможем сохранять некоторые
переменные в замыкании (например, чтобы посчитать время выполнения пинга).
